...
> **提示：** 原生 Windows 可直接运行 Hermes，不需要 WSL。CLI、gateway、TUI 和工具都支持原生 Windows。如果你更想使用 WSL2，也可以在 WSL2 中运行上面的 Linux/macOS 安装命令。
...
安装程序会处理 uv、Python 3.11、Node.js、ripgrep、ffmpeg，以及便携 Git Bash（MinGit，解压到 `%LOCALAPPDATA%\hermes\git`，无需管理员权限，不会干扰系统 Git）。Hermes 会使用这个 Git Bash 执行 shell 命令。

如果已经安装了 Git，安装程序会检测并使用现有 Git；否则只需下载约 45MB 的 MinGit。

> **Android / Termux：** 已测试的手动安装路径请参考 [Termux 指南](https://hermes-agent.nousresearch.com/docs/getting-started/termux)。在 Termux 上，Hermes 会安装精选的 `.[termux]` 扩展，因为完整的 `.[all]` 扩展会拉取 Android 不兼容的语音依赖。
>
> **Windows：** 原生 Windows 安装位于 `%LOCALAPPDATA%\hermes`；WSL2 安装位于 Linux 环境中的 `~/.hermes`。
