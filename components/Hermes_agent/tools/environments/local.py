...
def _find_bash() -> str:
    ...
    # PATH may contain the zero-byte WindowsApps bash.exe launcher for WSL.
    # It is not Git Bash and cannot consume native Windows working-directory
    # semantics used by the local backend, so consult PATH only after every
    # known Git for Windows location.
    found = shutil.which("bash")
    if found and "\\windowsapps\\bash.exe" not in found.lower():
        return found
