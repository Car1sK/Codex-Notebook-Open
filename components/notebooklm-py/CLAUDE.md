...
The Open Notebook bridge is a separate local boundary: `interchange.py` defines schema-v1 notebook bundles, `open_notebook.py` is a typed HTTP client, `notebooklm open-notebook ...` manages Open Notebook notes directly, and `python -m notebooklm.mcp.open_notebook` exposes only six Open Notebook tools without requiring Google authentication. `notebooklm mcp install-codex` registers that standalone server.
