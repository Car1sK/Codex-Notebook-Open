...
### Open Notebook bridge

This checkout includes a local bridge for explicit interchange with Open Notebook:

```bash
# Export a Google NotebookLM notebook to the versioned JSON contract
notebooklm export-bundle notebook.json -n NOTEBOOK_ID

# Publish a bundle back to Google NotebookLM
notebooklm publish-bundle notebook.json

# Manage notes on a running Open Notebook server
notebooklm open-notebook notebooks --json
notebooklm open-notebook notes --notebook-id notebook:ID --json

# Register the Open Notebook-only MCP server in Codex
notebooklm mcp install-codex
```

The direct Open Notebook client reads `OPEN_NOTEBOOK_URL` and `OPEN_NOTEBOOK_PASSWORD`. It does not require NotebookLM login. Bundle import into Open Notebook is previewable and idempotent; Google authentication remains on the notebooklm-py side.
