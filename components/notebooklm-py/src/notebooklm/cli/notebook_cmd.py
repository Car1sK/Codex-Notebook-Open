...
from ..interchange import NotebookBundle, export_notebook_bundle, publish_notebook_bundle
...
def register_notebook_commands(cli):
    ...
    @click.option(
        ...
        is_flag=True,
        help="Export source metadata without fetching full text.",
    )
    @click.option(
        "--max-concurrency",
        ...
        default=4,
        show_default=True,
        help="Maximum concurrent source-content requests.",
    )
    @json_option
    @with_client
    def export_bundle_cmd(
        ...
        output,
        notebook_id,
        without_source_content,
        max_concurrency,
        json_output,
        client_auth,
    ):
        """Export a notebook bundle for Open Notebook or other consumers."""
        notebook_id = require_notebook(notebook_id)
        ...
        async def _run():
            async with resolve_client_factory(ctx)(client_auth) as client:
                ...
                bundle = await export_notebook_bundle(
                    client,
                    resolved_id,
                    include_source_content=not without_source_content,
                ...
                if json_output:
                    json_output_response(payload)
                    return
                cli_print(f"[green]Exported notebook bundle:[/green] {output}", ctx=ctx)
                cli_print(
                    f"[dim]{len(bundle.sources)} sources, {len(bundle.notes)} notes[/dim]",
                    ctx=ctx,
                )

        return _run()

    @cli.command("publish-bundle")
    @click.argument(
        "bundle_file",
        type=click.Path(path_type=Path, exists=True, dir_okay=False, readable=True),
    )
    @click.option("--title", default=None, help="Override the bundle notebook title.")
    @json_option
    @with_client
    def publish_bundle_cmd(ctx, bundle_file, title, json_output, client_auth):
        """Publish an Open Notebook bundle as a new NotebookLM notebook."""
        bundle = NotebookBundle.from_json(bundle_file.read_text(encoding="utf-8"))

        async def _run():
            async with resolve_client_factory(ctx)(client_auth) as client:
                result = await publish_notebook_bundle(client, bundle, title=title)
                payload = {
                ...
                if json_output:
                    ...
                    return
                cli_print(
                    f"[green]Published NotebookLM notebook:[/green] {result.notebook_id}",
                    ctx=ctx,
                )
                cli_print(
                    f"[dim]{len(result.source_ids)} sources, "
                    f"{len(result.note_ids)} notes[/dim]",
                    ctx=ctx,
                )
