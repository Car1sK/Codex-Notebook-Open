...
from ..open_notebook import (
    DEFAULT_OPEN_NOTEBOOK_PASSWORD,
    DEFAULT_OPEN_NOTEBOOK_URL,
)
...
@mcp.command(name="install-codex")
@click.option("--name", default=SERVER_KEY, show_default=True)
@click.option(
    "--open-notebook-url",
    default=lambda: os.environ.get("OPEN_NOTEBOOK_URL", DEFAULT_OPEN_NOTEBOOK_URL),
    show_default=DEFAULT_OPEN_NOTEBOOK_URL,
)
@click.option(
...
def install_codex(
    ...
    """Install this local notebooklm-py build as a Codex MCP server."""
    codex_executable = _codex_executable()
    get_result = subprocess.run(
        [codex_executable, "mcp", "get", name],
        capture_output=True,
        text=True,
        check=False,
    )
    if get_result.returncode == 0:
        ...
        if remove_result.returncode != 0:
            raise click.ClickException(  # cli-input-validation: external Codex CLI rejected replacement
                remove_result.stderr.strip() or "Codex MCP removal failed"
            )
    ...
    command = [
        ...
        name,
        "--env",
        f"OPEN_NOTEBOOK_URL={open_notebook_url}",
        "--env",
        f"OPEN_NOTEBOOK_PASSWORD={open_notebook_password}",
        "--",
        sys.executable,
        "-m",
        "notebooklm.mcp.open_notebook",
    ]
    ...
    if add_result.returncode != 0:
        raise click.ClickException(  # cli-input-validation: external Codex CLI rejected installation
            add_result.stderr.strip() or "Codex MCP installation failed"
        )
    console.print(
        f"[green]Installed[/green] local notebooklm-py MCP server for Codex as "
        f"[cyan]{name}[/cyan]. Restart Codex to load it."
    )
...
def _codex_executable() -> str:
    ...
    for candidate in candidates:
        ...
        if resolved:
            return resolved
    raise click.ClickException(  # cli-input-validation: required local executable is unavailable
