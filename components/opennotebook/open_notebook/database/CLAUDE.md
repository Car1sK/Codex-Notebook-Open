...
- **NotebookLM bundle bridge** (`integrations/notebooklm.py`): migration 16 creates `external_sync_mapping`, keyed uniquely by provider, remote type, and remote ID. It stores only entity mappings and content hashes, never Google credentials.

Migration 16 also conditionally defines `credential.config` for compatibility with local databases that used migration number 15 before Open Notebook v1.10.0 assigned that version upstream.
