...
`routers/integrations.py` exposes the local NotebookLM bundle boundary:

- `POST /api/integrations/notebooklm/bundles/preview` validates a schema-v1 Google NotebookLM bundle and reports create/update/skip actions.
- `POST /api/integrations/notebooklm/bundles/import?embed_sources=false` performs an idempotent import and returns local IDs plus any embedding command IDs.
- `GET /api/integrations/notebooklm/notebooks/{notebook_id}/bundle` exports an Open Notebook notebook using the shared bundle contract.

The API accepts only `origin=google-notebooklm` for imports. Google cookies and profiles remain owned by `notebooklm-py`; they are never stored here.
