...
- Use `OpenNotebookLM.bat --setup-only` to install or repair missing dependencies without starting services and `OpenNotebookLM.bat --force-setup` to force setup checks without reinstalling already-present environments. Use `OpenNotebookLM.bat --check` or `--check-install` for package/setup readiness checks that do not require live services, and `OpenNotebookLM.bat --check-live` for the running stack check.
